...
export const getNotifications = async (userId: string) => {
  try {
    ...
    // Build query to fetch notifications relevant to the user's department and class
    let query = supabase
      .from("notifications")
      .select(`
        *,
    ...
    // Filter notifications based on user's department and class or notifications sent to all
    query = query.or(`send_to_all.eq.true,and(department_id.eq.${profile.department_id},assigned_class.eq.${profile.assigned_class}),and(department_id.eq.${profile.department_id},assigned_class.is.null),and(department_id.is.null,assigned_class.is.null)`);
    
    const { data, error } = await query;
    
    if (error) throw error;
    
    // Determine which notifications have been read by the user
    return data.map(notification => ({
      ...notification,
      department: notification.departments?.name,
      is_read: notification.notifications_read && notification.notifications_read.length > 0
    }));
  } catch (error) {
...
export const getNotification = async (id: string, userId: string) => {
  try {
    const { data, error } = await supabase
      .from("notifications")
      .select(`
        *,
        departments:department_id(name),
        notifications_read!inner(user_id)
      `)
      .eq("id", id)
      .eq("notifications_read.user_id", userId)
      .maybeSingle();
    ...
    if (data) {
      return {
        ...data,
        department: data.departments?.name,
        is_read: data.notifications_read && data.notifications_read.length > 0
      };
    }
    
    // If no rows returned with the join, fetch the notification without checking read status
    ...
    return {
      ...notificationData,
      department: notificationData.departments?.name,
      is_read: false
    };
  } catch (error) {
